...
const geminiClient = new GeminiClient({
  apiKey: process.env.GEMINI_API_KEY, // Use the API key from environment variables
});
 ...
 */
async function chatWithGemini(message = "", socket) {
  try {
    ...
    const geminiResponse = await geminiClient.chat.completions.create({
      model: "gemini-model", // Use the appropriate model name for Gemini
      messages: socket.data.chat_history,
    });
    const data = geminiResponse.choices[0].message.content;
    ...
    return {
      isSuccessful: true,
      message: "Received response from Gemini.",
      data: data,
    }
  } catch (err) {
    return {
      isSuccessful: false,
      message: "Error processing your request with Gemini. Here is the error:\n" + err,
      data: null,
    }
  }
}
...
async function interactionPipelineLoop(audioData, selectedInterviewer, socket) {
  ...
  const chatReponse = await chatWithGemini(transcribeAudioFileResponse.transcription, socket);
  if (!chatReponse.isSuccessful) {
    console.error(chatReponse.message);
    return;
  }
  console.debug(chatReponse.message);
  console.info("Gemini Response:", chatReponse.data);
...
async function handleStartInterview(resume, jobDescription, selectedInterviewer, socket) {
  ...
  // Start the interview
  const chatReponse = await chatWithGemini("", socket);
  if (!chatReponse.isSuccessful) {
    console.error(chatReponse.message);
    return;
  }
...
async function handleEndInterview(socket) {
  ...
  const chatReponse = await chatWithGemini("Now that the interview is over, I want you to generate a report of the interview.\
